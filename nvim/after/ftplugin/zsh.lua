...
vim.keymap.set("n", "<F9>", function()
    ...
    if vim.fn.executable("zsh") == 0 then
        vim.notify("zsh not found!", vim.log.levels.ERROR)
        return
    end
    ...
    -- Run zsh script
    vim.cmd("terminal zsh " .. vim.fn.shellescape(filepath))
    vim.cmd("startinsert")
end, { buffer = true, silent = true, desc = "Run zsh script" })

-- Keymapping for shellcheck (F8) - works with zsh too
vim.keymap.set("n", "<F8>", function()
    ...
    local filepath = vim.fn.expand("%:p")
    local output = vim.fn.system("shellcheck -s bash " .. vim.fn.shellescape(filepath))
...
end, { buffer = true, silent = true, desc = "Run shellcheck on zsh script" })
...
-- Source current zsh file (useful for config files)
vim.keymap.set("n", "<leader>s", function()
    local filepath = vim.fn.expand("%:p")
    vim.fn.system("zsh -c 'source " .. vim.fn.shellescape(filepath) .. "'")
    vim.notify("Sourced: " .. filepath, vim.log.levels.INFO)
end, { buffer = true, silent = true, desc = "Source zsh file" })
