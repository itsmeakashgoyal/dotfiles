...
vim.keymap.set("n", "<leader>tf", ":!touch %<cr>", {
    silent = true,
    noremap = true,
    desc = "touch file to reload observers"
})
utils.nmap("<nop>", "<Plug>NERDCommenterAltDelims") -- tab is for moving around only
vim.api.nvim_set_keymap("n", "<leader>tv", ":vsp term://", {
    noremap = true,
    silent = false
})
vim.api.nvim_set_keymap("n", "<leader>th", ":sp term://", {
    noremap = true,
    silent = false
})
utils.nmap("L", "vg_", {
...
vim.keymap.set("n", "<leader>_", "5<c-w>-", {
    remap = true,
    silent = false
})
...
vim.keymap.set("n", "<A-m>", nvim_tmux_nav.NvimTmuxNavigateNext, {
    noremap = true,
    silent = true
})

-- Insert mode mappings
vim.keymap.set("i", "<C-h>", [[<C-\><C-N>:lua require("nvim-tmux-navigation").NvimTmuxNavigateLeft()<CR>]], {
    noremap = true,
    silent = true
})
vim.keymap.set("i", "<C-j>", [[<C-\><C-N>:lua require("nvim-tmux-navigation").NvimTmuxNavigateDown()<CR>]], {
    noremap = true,
    silent = true
})
vim.keymap.set("i", "<C-k>", [[<C-\><C-N>:lua require("nvim-tmux-navigation").NvimTmuxNavigateUp()<CR>]], {
    noremap = true,
    silent = true
})
vim.keymap.set("i", "<C-l>", [[<C-\><C-N>:lua require("nvim-tmux-navigation").NvimTmuxNavigateRight()<CR>]], {
    noremap = true,
    silent = true
})
vim.keymap.set("i", "<A-m>", [[<C-\><C-N>:lua require("nvim-tmux-navigation").NvimTmuxNavigateNext()<CR>]], {
    noremap = true,
    silent = true
})
...
vim.keymap.set("t", "<A-m>", "<C-\\><C-n><A-m>", {
    noremap = true,
    silent = true
})
...
vim.keymap.set({"n", "v"}, "<A-j>", [[10j<cr>]], {
    desc = "moves over virtual (wrapped) lines down"
})
vim.keymap.set({"n", "v"}, "<A-k>", [[10k<cr>]], {
...
vim.api.nvim_set_keymap("n", "<Mgo-Right>", "gT", {
    noremap = true,
    silent = true,
...
utils.vmap("<S-PageDown>", ":m '>+1<CR>gv=gv", {
    desc = "Move Line Down in Visual Mode"
})
utils.vmap("<S-PageUp>", ":m '<-2<CR>gv=gv", {
    desc = "Move Line Up in Visual Mode"
})
utils.nmap("<leader>k", ":m .-2<CR>==", {
    desc = "Move Line Up in Normal Mode"
})
utils.nmap("<leader>j", ":m .+1<CR>==", {
    desc = "Move Line Down in Normal Mode"
})
utils.nmap("<Leader>em", ":/\\V\\c\\<\\>", {
...
utils.vmap("<leader><C-s>", ":s/\\%V", {
    desc = "Search only in visual selection usingb%V atom"
})
utils.vmap("<C-r>", '"hy:%s/\\v<C-r>h//g<left><left>', {
    silent = false,
...
vim.keymap.set("n", "<leader>ml", "^I-<Space>[<Space>]<Space><Esc>^j", {
    remap = true,
    silent = false,
...
utils.nmap("<leader>is", "i<space><esc>", {
    desc = "Insert space in normal mode"
})
utils.nmap("<leader>sq", ':normal viWS"<CR>', {
...
utils.nmap("<leader>0", '"0p', {
    desc = "paste from 0 (latest yank)"
})
utils.nmap("<leader>1", '"1p', {
    desc = "paste from 1 (latest delete)"
})
utils.nmap("<leader>2", '"*p', {
...
vim.keymap.set("v", "<leader>gc", ":GBrowse!<CR>", {
    noremap = true,
    silent = false
...
-- GoTo Preview
vim.keymap.set("n", "gtp", "<cmd>lua require('goto-preview').goto_preview_definition()<CR>", {
    noremap = true
})

-- Yank Matching Lines
vim.api.nvim_set_keymap("n", "<Leader>ya", ":YankMatchingLines<CR>", {
    noremap = true,
    silent = true
})

-- Noice
vim.api.nvim_set_keymap("n", "<leader>nn", ":Noice dismiss<CR>", {
    noremap = true
})
