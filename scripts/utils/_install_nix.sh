...
#  ▓▓▓▓▓▓▓▓▓▓
# ░▓ author ▓ Akash Goyal
# ░▓ file   ▓ scripts/utils/_install_nix.sh
# ░▓▓▓▓▓▓▓▓▓▓
# ░░░░░░░░░░
#
#█▓▒░
...
install_nix() {
	...
	if [[ -f "$NIX_ENV_FILE" ]]; then
		...
		error "Nix environment file not found"
		return 1
	fi
}
...
setup_nix_config() {
	...
	if ! cp -rf "${DOTFILES_DIR}/nix" "$CONFIG_DIR"; then
		error "Failed to copy Nix configuration"
		return 1
	fi

	# Change to nix config directory
	if ! cd "${CONFIG_DIR}/nix"; then
		error "Failed to change to Nix config directory"
		return 1
	fi
}
...
setup_home_manager() {
	...
	if [ -z "${CI:-}" ]; then
		...
		if ! nix run home-manager -- init --switch .; then
			error "Failed to initialize Home Manager"
			return 1
		fi

		# Switch to Home Manager configuration
		if ! home-manager switch --flake .; then
			error "Failed to switch Home Manager configuration"
			return 1
		fi
	fi
}
...
main() {
	...
	success "Nix installation and setup completed successfully!"
}
